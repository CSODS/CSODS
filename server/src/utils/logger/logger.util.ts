...
const { combine, errors, json, timestamp, prettyPrint, align, printf } =
  winston.format;
...
export const ProjectsCacheLogger = winston.createLogger({
  ...
  format: combine(
    ...
    align(),
    printf(
      (info) => `[cache] [${info.timestamp}] ${info.level}: ${info.message}`
    )
  ),
  transports: [
...
export const JobsLogger = winston.createLogger({
  ...
  format: combine(
    ...
    align(),
    printf(
      (info) => `[cron] [${info.timestamp}] ${info.level}: ${info.message}`
    )
  ),
  transports: [
    new winston.transports.Console(),
...
export const FileLogger = winston.createLogger({
  ...
  format: combine(
    ...
    align(),
    printf(
      (info) => `[file] [${info.timestamp}] ${info.level}: ${info.message}`
    )
  ),
  transports: [
...
export const DbLogger = winston.createLogger({
  ...
  format: combine(
    ...
    align(),
    printf(
      (info) => `[database] [${info.timestamp}] ${info.level}: ${info.message}`
    )
  ),
  transports: [
    new winston.transports.Console({ level: "error" }),
