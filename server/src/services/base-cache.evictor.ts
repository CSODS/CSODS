...
export class BaseCacheEvictor<TCache extends ICache> {
  ...
  public constructor(
    ...
    this._defaultEvictionOptions = this.normalizeEvictionOptions(
      defaultEvictionOptions
    );
  }
  /**
   * @public
  ...
  protected async readFile(file: IFile): Promise<TCache | null> {
    ...
    const data = await this._jsonFileHandler.parseJsonFile(
      filepath,
      filename,
      this.reviver
    );
   ...
   */
  protected normalizeEvictionOptions(
    evictionOptions?: IEvictionOptions
  ): IEvictionOptions {
