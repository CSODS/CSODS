...
import { createJsonFileHandler, createProjectDataService, JsonFileHandler, ProjectDataService } from "@services";
...
export async function createProjectCachePageService() {
    ...
    return new ProjectCachePageService(projectDataServiceInstance, jsonFileHandlerInstance);
}
...
export class ProjectCachePageService extends ProjectCacheService {
    ...
    public async getProjectByPageAndId(pageNumber: number, projectId: number) : Promise<IProjectDetails | null> {
        ...
        this._logger.info(`Attempting to retrieve project with id: ${projectId}...`);
    ...
    public async getOrCreatePage(pageNumber: number): Promise<IProjectCachePage | null> {
        ...
        {  
            ...
            if (isPageOutOfBounds(this._cache.TotalPages, pageNumber)) {
                this._logger.warn(`Page ${pageNumber} is out of bounds. Returning null...`);
                return null;
            }
            ...
            if (this.isPageMissingFromCache(cachePages, pageNumber)) {
                if (this._cache.IsBackup) {
                    this._logger.warn(`Page ${pageNumber} is not available in backup cache. Returning null...`);
                    return null;
                }
     ...
     */
    private async setNewCachePage(pageNumber: number) : Promise<IProjectCachePage> {
        try 
        {
            ...
            await this._jsonFileHandler.writeToJsonFile(process.env.PROJECT_CACHE_PATH!, this._filename, this._cache);
        ...
        catch (err)
        {
            ...
            if (this._cache != null && !this.isPageMissingFromCache(this._cache.CachePages, pageNumber)) {
