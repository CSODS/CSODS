 ...
 * - {@link _jsonFileHandler} - An instance of the {@link JsonFileHandler} class. A core component for the default
...
export abstract class AbstractCacheService<TCache extends ICache> {
     ...
     * @public
     * @constructor
     * @description The constructor. Accepts parameters of type {@link JsonFileHandler} and {@link string}.
     ...
     */
    public constructor(logger: winston.Logger, jsonFileHandler: JsonFileHandler<TCache>, cachePath: string)
    {
        this._logger = logger;
    ...
    protected async parseCache(filePath?: string): Promise<TCache | null> {
        ...
        const cache = await this._jsonFileHandler.parseJsonFile(cachePath, filename, reviver);
        return cache;
    }
    /**
     * @protected
     ...
     * {@link JsonFileHandler.parseJsonFile}.
     * 
    ...
    protected async tryCreateCache(filePath?: string): Promise<TCache | null> {
        ...
        try {
            ...
            const savedCache = await this._jsonFileHandler.writeToJsonFile(cachePath, filename, newCache);
