...
export class LegacyProjectCacheService extends LegacyAbstractCacheService<IProjectCache> {
   ...
   */
  public override async setCache(
    ...
    const cachedProjects: IProjectCache | null =
      await this.tryParseOrCreateCache();
   ...
   */
  protected override getFilename(options?: {
    ...
    const hasSearchKey =
      this._filter !== undefined && this._filter?.hasProjectSearchKey();
  ...
  protected override async constructCache(): Promise<IProjectCache> {
    ...
    function createCachePages(
      ...
      recordEntries.map((keyPagePair) => {
        ...
        cachePages[numericKey] = createCachePage(
          date,
          totalPages,
          keyPagePair[1]
        );
    ...
    const projectCache: IProjectCache = createCache(
      totalPages,
      this._cDate,
      cachePages
    );
  ...
  private async parseBackupCache(): Promise<IProjectCache | null> {
    ...
    const hardBackupCache: IProjectCache | null = await this.parseCache(
      hardBackupPath
    );
