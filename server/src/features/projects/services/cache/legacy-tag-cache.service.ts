...
import { JsonService, createJsonService } from "@services";
...
/**
 * @deprecated Please use `createProjectTagsCacheService`
 * @returns
 */
export function createLegacyTagsCacheHandler() {
  const jsonFileHandlerInstance =
    createJsonService<IProjectTagsCache>("IProjectTags");
  return new LegacyTagsCacheHandler(jsonFileHandlerInstance);
}

/**
 * @deprecated Please use `ProjectTagsCacheService`
 */
export class LegacyTagsCacheHandler {
  private readonly _jsonFileHandler: JsonService<IProjectTagsCache>;
  private readonly _filepath: string;
  private readonly _filename: string;
  private _tagsCache: IProjectTagsCache | null = null;

  public constructor(jsonFileHandler: JsonService<IProjectTagsCache>) {
