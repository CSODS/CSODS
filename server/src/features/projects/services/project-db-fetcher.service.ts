...
/**
 * Service responsible for retrieving project data from the database via the
 * `ProjectRepository`. Provides utility methods for fetching individual or
...
export class ProjectDbFetcher {
   ...
   *
   * @param projectRepository The repository instance used for `projects`
   * table query handling.
   * @param projectFrameworkRepository Repository instance used for
   * `project_frameworks` table query handling.
   */
  public constructor(
  ...
  /**
   * Fetches one or more pages of project data from the database and organizes
   * the results into a page-number-keyed object.
   *
   * Each page consists of a list of `IProjectDetails` entries, combining the
   * core project data (`Project`) with its associated frameworks
   * (`ProjectFrameworks`). Pages are retrieved in ascending order starting from
   * `pageStart`.
   *
   * The operation stops early if an empty page is returned — this is treated as
   * an indicator that there are no more projects beyond that point in the
   * database.
   *
   * ### Behavior:
   * - Pages are 1-indexed.
   ...
   * - `pageEnd` (number, optional): The last page to fetch (inclusive).
   ...
   * where each key is a page number
   * and each value is an array of fully-hydrated `IProjectDetails` entries
   * (project + frameworks).
   *
   * @example
  ...
  public async fetchProjectsPages(options: {
    ...
    // Calculate the logical start and end row numbers for logging purposes,
    // based on the page range and page size.
    ...
    for (let pageNumber = pageStart; pageNumber <= pageEnd; pageNumber++) {
      ...
      //  If no projects are returned for the current page, it signifies the
      //  end of available data, so stop fetching further pages.
      if (projectArr.length == 0) {
        DbLogger.info(
          `[fetchProjectsPages] No rows found at page ${pageNumber}. Ending fetch...`
        );
        break;
      }
    ...
    DbLogger.info(`[fetchProjectsPages] Finished fetching pages.`);
    return projectDetails;
  }
  /**
   * Constructs an array of `IProjectDetails` objects from the given list of
   * `Project` entities.
   *
   * For each project, this method fetches its associated frameworks
   * asynchronously and combines them into a single `IProjectDetails` object.
   *
   * @param projectArr An array of `Project` entities for which to fetch
   * associated frameworks.
   * @returns A promise resolving to an array of fully-hydrated `IProjectDetails`
   * objects, where each entry contains the project and its associated
   * frameworks.
   *
   * @example
