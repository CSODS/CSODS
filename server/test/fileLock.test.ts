...
import { IProjectCache } from "../src/viewmodels/cache/cache.interface";
...
describe("fileHandler", async () => {
  ...
  const reviver: Parameters<typeof JSON.parse>[1] = (key, value) => {
    if (key === "LoadTime" && typeof value === "string") return new Date(value);
    return value;
  };
  ...
  it("reads JSON from file", async () => {
    const data = await _jsonFileHandler.parseJsonFile(
      "cache/",
      "fake.json",
      reviver
    );
  ...
  it("writes JSON to file", async () => {
    ...
    const content = await _jsonFileHandler.parseJsonFile(
      "cache/",
      "fake.json",
      reviver
    );
