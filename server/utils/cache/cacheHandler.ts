...
import { IProjectCachePage, IProjectCache, ProjectCachePages } from '../../viewmodels/cache/cacheInterfaces.js';
import { Project, DbContext } from '../../viewmodels/dbModels.js';
...
import { ProjectCache } from '../../viewmodels/cache/cacheModels.js';
 ...
 */
export async function getProjectsCache(): Promise<Project[]> {
    //  Get current date.
...
async function writeCacheToJson(filePath: string) {
    ...
    const csodsContext = await createContext();
...
export class ProjectCacheHandler {
     ...
     * @throws {Error} If the JSON cache is not loaded.
     */
    public getCachePages(): ProjectCachePages {
        this.assertCacheLoaded(this._jsonCache);
        return this._jsonCache!.CachePages;
    }

    public async getProject(pageNumber: number, projectId: number) : Promise<Project | null> {
        const cachePage: IProjectCachePage | null = await this.getJsonCachePage(pageNumber);
        if (cachePage != null) {
            return cachePage.ProjectList.find(p => p.ProjectId == projectId) ?? null;
        }
        return null;
    }
    /**
    ...
    public async setProjectsCache() : Promise<void> {
        ...
        let cachePages: ProjectCachePages = {}

        //  Attempt to read cache from stored Json.
        console.log('Attempting to parse Json cache and store in memory...');
        cachedProjects = await this.tryParseOrCreateJsonCache(filePath);
        ...
        if (cachedProjects == null || Object.entries(cachePages).length == 0) {
            ...
            cachedProjects = await this.parseBackupJsonCache(cDate);
        }
    ...
    private async setNewJsonCachePage(pageNumber: number) : Promise<IProjectCachePage> {
        try 
        {
            ...
            const filePath: string = this.generateJsonPath(process.env.PROJECT_CACHE_PATH!, CACHE.BASE_NAME, new Date());
            //  throws an exception if update fails.
            await this.writeToJsonFile(filePath, this._jsonCache);
     ...
     */
    private isPageInJsonCache(cachePages: ProjectCachePages, pageNumber: number): boolean {
    ...
    private async generateProjectCache(): Promise<IProjectCache> {
        ...
        const cachePages: ProjectCachePages = await this.fetchCachePages(1, 3);

        //  fetch total project count from database.
        console.log('Fetching total projects count from database.');
        const projectsCount: number = await this._projectRepository.GetCount();
     ...
     */
    private async tryParseOrCreateJsonCache(filePath: string): Promise<IProjectCache | null> {
        //  Attempt to read cache from stored Json file.
        let cachedProjects: IProjectCache | null = await this.parseJsonCache(filePath);
        ...
        if (cachedProjects == null) {
            ...
            for (let i = 0; i < 3; i++) {
                try {
                    ...
                    return await this.createNewJsonCache(filePath);
                }
                catch {
    ...
    private async parseBackupJsonCache(cDate: Date): Promise<IProjectCache | null> {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            cachedProjects = await this.parseJsonCache(backupPath);
        ...
        const hardBackupPath: string = this.generateJsonPath(process.env.DEFAULT_CACHE_PATH!, CACHE.HARD_BACKUP);
        const hardBackupCache: IProjectCache | null = await this.parseJsonCache(hardBackupPath);
     ...
     */
    private async parseJsonCache(filePath: string): Promise<IProjectCache | null> {
        //  Read from json file
        try {
            ...
            const projectsJson: IProjectCache = this.cacheReviver(data);
     ...
     */
    private async createNewJsonCache(filePath: string): Promise<IProjectCache> {
        ...
        return await this.writeToJsonFile(filePath, projectCache);
    }
    /**
     ...
     */
    private async writeToJsonFile(filePath: string, projectCache: IProjectCache | null): Promise<IProjectCache> {
        try {
     ...
     */
    private assertCacheLoaded(cache: IProjectCache | null) {
        if (cache == null)
     ...
     */
    private cacheReviver(data: string) : IProjectCache {
     ...
     */
    private async fetchCachePages(pageStart: number, pageEnd: number): Promise<ProjectCachePages> {
